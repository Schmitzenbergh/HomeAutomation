...
//--------------------------------------------------------------------------
int ReadJpegSections (unsigned int infile, ReadMode_t ReadMode)
{
    int a;
    int HaveCom = FALSE;
    ...
    for(;;){
        ...
        got = JFREAD(infile, Data+2, itemlen-2); // Read the whole section.
        if (got != itemlen-2){
        ...
        switch(marker){
            ...
            case M_SOS:   // stop before hitting compressed data 
                ...
                if (ReadMode & READ_IMAGE){
                    ...
                    cp = JFTELL(infile);
                    JFSEEK(infile, 0, SEEK_END);
                    ep = JFTELL(infile);
                    JFSEEK(infile, cp, SEEK_SET);
...
int ReadJpegFile(const char * FileName, ReadMode_t ReadMode)
{
    ...
    infile = JFOPEN(FileName, "rb"); // Unix ignores 'b', windows needs it.
...
int SaveThumbnail(char * ThumbFileName)
{
    ...
    if (strcmp(ThumbFileName, "-") == 0){
        ...
        ThumbnailFile = JFOPEN(ThumbFileName,"wb");
    }
    ...
    if (ThumbnailFile){
        ...
        JFWRITE(ThumbnailFile, ThumbnailPointer, ImageInfo.ThumbnailSize);
        JFCLOSE(ThumbnailFile);
        return TRUE;
    }else{
...
int ReplaceThumbnail(const char * ThumbFileName)
{
    ...
    if (ThumbFileName){
        ThumbnailFile = JFOPEN(ThumbFileName,"rb");
        ...
        // get length
        JFSEEK(ThumbnailFile, 0, SEEK_END);

        ThumbLen = JFTELL(ThumbnailFile);
        JFSEEK(ThumbnailFile, 0, SEEK_SET);
    ...
    if (ThumbnailFile){
        JFREAD(ThumbnailFile, ThumbnailPointer, ThumbLen);
        JFCLOSE(ThumbnailFile);
    }
...
void WriteJpegFile(const char * FileName)
{
    ...
    for (a=0;a<SectionsRead-1;a++){
        ...
        JFWRITE(outfile, Sections[a].Data, Sections[a].Size);
    }
