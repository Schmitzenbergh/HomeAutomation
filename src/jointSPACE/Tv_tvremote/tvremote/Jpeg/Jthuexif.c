...
//JMH            ProcessMakerNote(ValuePtr, ByteCount, OffsetBase, ExifLength);
            continue;
        }
...
//JMH                        ProcessGpsInfo(SubdirStart, ByteCount, OffsetBase, ExifLength);
                    }
                    continue;
                }
                break;
...
void ShowImageInfo(int ShowFileInfo)
{
    ...
    if (ImageInfo.Comments[0]){
        ...
        if (!ImageInfo.CommentWidthchars){
            ...
            printf("%.*ls\n", ImageInfo.CommentWidthchars, ImageInfo.Comments);
        }
    }
}
