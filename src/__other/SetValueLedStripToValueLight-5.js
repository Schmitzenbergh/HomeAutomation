...
function initWaitForUpdate(){
	...
	if ( bRgbDiffStatus == false) {
		...
		if (iWaitForUpdate_LightGetRGB < 2){
			...
			if (tickrate < tickrateLimit){
				tickrate += 1; 
			}
		}
	} else if (bRgbDiffStatus == true) {	
		if (tickrate > tickrate.default && tickrate > 5){
			tickrate -= 1;
		}
    ...
    var fetchStatusLightGetRGB = function(iLightNumber,callback) {
        api.lightStatusWithRGB(iLightNumber, function(err, result) {
            ...
            if (err) {
				...
				if (tickrate < ( tickrateLimit - 5 ) ){
					tickrate += 5;
				}
				throw err;
			}
			
			process.stdout.write('\n' + "1a: " + result.state.rgb[0] + " " + "1b: " + result.state.rgb[1] + " " + "1c: " + result.state.rgb[2] + " ");
			
			if (tickrate > ( tickrate.default + 50 ) ){
				tickrate -= 50;
			}			
...
//	function statusLightGetRGB(iLightNumber){
//	    var statusLightGetRGBCallback = function(data) {}; //With callback
//	    var fetchStatusLightGetRGB = function(iLightNumber,callback) {
//	        api.lightStatusWithRGB(iLightNumber, function(err, result) {
//	            if (err) throw err; 
//				//callback(JSON.stringify(result.state.rgb, null, 2));
...
//	        });
//	    };
//	    fetchStatusLightGetRGB(iLightNumber, statusLightGetRGBCallback);
//	}
...
function LightTransitionDifferenceFade(){
	...
	const stepValue = [ 7, 3, 1];
		...
		cLogStdout(" Done! with ms:" + tickrate );
}
